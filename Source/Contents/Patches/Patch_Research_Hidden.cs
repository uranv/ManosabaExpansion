...
public static class Patch_ResearchVisibility
{
    public static void Postfix(ResearchProjectDef __instance, ref bool __result)
    {
        ...
        // 隐藏前置科技
        if (__instance == ModDefOf.UmResearchProjectPrereqs)
        {
            __result = !__instance.IsFinished;
        }

        // 隐藏【检查魔女的方法】
        if (__instance == ModDefOf.UmResearchProjectDetect)
        {
            __result = !__instance.PrerequisitesCompleted;
        }
        ...
        // 隐藏【魔女因子基因】
        if (__instance == ModDefOf.UmResearchProjectGene)
        {
            __result = (ModDefOf.UmResearchProjectKill == null || !ModDefOf.UmResearchProjectKill.IsFinished) &&
